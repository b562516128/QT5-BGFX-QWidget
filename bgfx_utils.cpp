...
{
	return load(BGFXWidget::fileReader(), BGFXWidget::allocator(), _filePath, _size);
}

void unload(void* _ptr)
{
	BX_FREE(BGFXWidget::allocator(), _ptr);
}
...
{
	return loadShader(BGFXWidget::fileReader(), _name);
}

bgfx::ProgramHandle loadAndCreateShaderProgram(bx::FileReaderI* _reader, const char* _vsName, const char* _fsName)
{
...
bgfx::TextureHandle loadTexture(bx::FileReaderI* _reader, const char* _filePath, uint64_t _flags, uint8_t _skip, bgfx::TextureInfo* _info, bimg::Orientation::Enum* _orientation)
{
	...
	uint32_t size;
	void* data = load(_reader, BGFXWidget::allocator(), _filePath, &size);
	if (NULL != data)
	{
		bimg::ImageContainer* imageContainer = bimg::imageParse(BGFXWidget::allocator(), data, size);
...
{
	return loadTexture(BGFXWidget::fileReader(), _name, _flags, _skip, _info, _orientation);
}

bimg::ImageContainer* imageLoad(const char* _filePath, bgfx::TextureFormat::Enum _dstFormat)
{
	uint32_t size = 0;
	void* data = loadMem(BGFXWidget::fileReader(), BGFXWidget::allocator(), _filePath, &size);

	return bimg::imageParse(BGFXWidget::allocator(), data, size, bimg::TextureFormat::Enum(_dstFormat) );
}
...
void Mesh::load(bx::ReaderSeekerI* _reader, bool _ramcopy)
{
	...
	bx::AllocatorI* allocator = BGFXWidget::allocator();
...
{
	bx::FileReaderI* reader = BGFXWidget::fileReader();
...
{
	MeshState* state = (MeshState*)BX_ALLOC(BGFXWidget::allocator(), sizeof(MeshState) );
	return state;
}

void meshStateDestroy(MeshState* _meshState)
{
	BX_FREE(BGFXWidget::allocator(), _meshState);
}
